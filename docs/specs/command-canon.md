...
| `selfheal` | Intentional | Rewritten — removed hardcoded `.claude/commands/` paths (Hard Law #1 violation); now checks the canon against the active platform's command directory generically. GETSITREP-28 (hash manifest, drift detection, lock/diff/restore) is fully landed: baseline manifest (GETSITREP-29), drift report (GETSITREP-30), and lock/diff/restore + upgrade protection (GETSITREP-31) |
